...
int lazy_active_classification() {
	...
	while(judgements<MAX_JUDGEMENTS) {
		...
		printf("total_occurs %d n_rules %d diff %d \n", ordered_tests[0].total_occurs, ordered_tests[0].n_rules, ordered_tests[0].diff);
		printf("total_occurs %d n_rules %d diff %d \n", ordered_tests[1].total_occurs, ordered_tests[1].n_rules, ordered_tests[1].diff);
